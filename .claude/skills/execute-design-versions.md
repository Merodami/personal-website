---
name: execute-design-versions
description: Master execution skill that builds all 10 portfolio design versions one by one. Creates a worktree for each version, implements the full design, verifies, and asks to commit. Invoke to start or resume the process.
user_invocable: true
---

# Skill: Execute Design Versions
...
| Order | Version | Branch Name | Skill File |
|-------|---------|-------------|------------|
...
1. **The version's skill file:** `.claude/skills/build-{name}.md` - This is the design specification.
2. **Full project overview:** `docs/overview/000-COMPLETE-PROJECT-OVERVIEW.md` - Complete project context.
3. **Key source files** (as listed in the skill file's Step 1):
...
Follow the skill file's implementation process exactly. Use sub-agents to parallelize independent work:
...
- Follow the skill file's Visual Identity section for colors, typography, spacing.
- Follow the skill file's Key Visual Effects section for animations and interactions.
...
Run ALL verification checks from the checklist:

```bash
# Build checks (run in parallel if independent)
yarn lint
yarn build        # includes astro check
yarn test

# Manual checks to report to user:
# - i18n switching (describe how to test)
# - Theme toggle (describe what to look for)
# - Mobile responsiveness (describe breakpoints)
...
2. Take a break (you can resume later by invoking this skill again)
```
...
When this skill is invoked and versions already exist:
...
- **Follow the skill file** - each version's `.claude/skills/build-{name}.md` is the design bible.
